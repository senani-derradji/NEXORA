...
# ---------------- STOP INFLUXDB WITHOUT REMOVING VOLUME ----------------
Write-Host "Stopping InfluxDB (keeping volume)..."
docker compose -f $ComposeFile --profile $Profile stop influxdb

# ---------------- REMOVE OLD ENV FILES ----------------
Write-Host "Removing old .env files..."
...
foreach ($file in $envFilesToRemove) {
    if (Test-Path $file) {
...
# ---------------- ENV FILES SETUP ----------------
Write-Host "Creating new .env files..."
...
docker compose -f $ComposeFile --profile $Profile up -d influxdb
